...
| Standard | Rule |
|---|---|
| **Governor limits** | 100 SOQL queries per synchronous transaction; 300 in async contexts |
| **Security** | use `WITH SECURITY_ENFORCED` or `WITH USER_MODE`; never expose un-stripped fields to callers |
| **No injection** | always use bind variables (`:`); never concatenate user input into query strings |
