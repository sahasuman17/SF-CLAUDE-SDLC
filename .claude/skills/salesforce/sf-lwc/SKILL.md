...
| Standard | Rule |
|---|---|
| **No hardcoding** | use Custom Labels (`$label.*`) for all user-facing strings; never inline static text |
| **Naming** | PascalCase for the JS class; kebab-case for element and file names |
| **Documentation** | JSDoc on every `@api` property and public method |
| **Security** | never expose record IDs in URL params; rely on LDS or Apex to enforce FLS |
