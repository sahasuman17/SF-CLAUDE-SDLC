...
## 11. Custom-Styled Buttons and Footer Layout (Learned from MP-7)

These four anti-patterns were discovered during the MP-7 `caseCreate` LWC build. Each caused a visible mismatch between the deployed component and the design mockup that required a second correction pass.

---

### 11a. Styling `lightning-button` internals with `::part()` — DOES NOT WORK in synthetic shadow

`::part()` is a CSS Shadow Parts spec selector. LWC's synthetic shadow DOM (Locker Service, used in most orgs) does **not** support it. Rules like `lightning-button[label="Save"]::part(button)` are silently ignored — no error, no effect.

#### ❌ BAD: `::part()` on `lightning-button`

```css
/* Looks correct but is completely ignored in synthetic shadow */
.footer-action-bar lightning-button[label="Save"]::part(button) {
    background-color: #ba0517;
    color: #ffffff;
}
.footer-action-bar lightning-button[label="Submit"]::part(button) {
    color: #ba0517;
    border-color: #ba0517;
}
```

#### ✅ GOOD: Replace `lightning-button` with native `<button>` and apply CSS classes directly

```html
<!-- caseCreate.html -->
<div class="footer-action-bar">
    <button class="footer-btn footer-btn-save" onclick={handleSave} disabled={isSaving} type="button">Save</button>
    <button class="footer-btn footer-btn-outline" onclick={handleSubmit} type="button">Submit</button>
    <button class="footer-btn footer-btn-outline" onclick={handleResolve} type="button">Resolve</button>
    <button class="footer-btn footer-btn-outline" onclick={handleCancel} type="button">Cancel</button>
</div>
```

```css
/* caseCreate.css — direct class selectors, no shadow boundary to cross */
.footer-btn {
    height: 2rem;
    padding: 0 1.25rem;
...
> **Rule:** Whenever you need custom background/border/text colors on a button that don't match any standard SLDS `variant`, use a native `<button>` element with CSS classes instead of `lightning-button`. This avoids all shadow-boundary styling issues.

---

### 11b. Circular CSS custom property — resolves to nothing

Defining a CSS custom property that references itself is a circular dependency. The browser resolves it to an empty value; every downstream rule using `var(--brand-red)` silently fails to apply any color.

#### ❌ BAD: Self-referencing variable

```css
:host {
    --brand-red: var(--brand-red);       /* circular — resolves to nothing */
    --brand-red-hover: var(--brand-red-hover); /* same problem */
}
```

#### ✅ GOOD: Always define custom properties with concrete values

```css
:host {
    --brand-red: #ba0517;
    --brand-red-hover: #8e0311;
}
```

> **Rule:** The `:host` block must define all custom properties with literal values (hex, rgb, etc.). Never use `var(--x)` on the right-hand side of the same variable's definition.

---

### 11c. Footer buttons left-aligned instead of centered

A flex container defaults to `justify-content: flex-start` (left-aligned). Forgetting `justify-content: center` leaves footer buttons pinned to the left, which doesn't match typical centered action-bar designs.

#### ❌ BAD: Missing `justify-content`
...
.footer-action-bar {
    ...
    align-items: center;
    gap: 0.75rem;
    /* buttons will be left-aligned */
}
```

#### ✅ GOOD: Explicitly set alignment

```css
.footer-action-bar {
    display: flex;
    align-items: center;
    justify-content: center;   /* ← required for centered buttons */
    gap: 0.75rem;
}
```

> **Rule:** Whenever a mockup shows a centered footer action bar, always include `justify-content: center` on the flex container. Default `flex-start` will produce a left-aligned result.

---

### 11d. Required field asterisks hidden in separate DOM elements instead of inline in label

Placing the asterisk in a separate `<div class="required-asterisk">*</div>` next to a `lightning-input` and then setting `display: none` on it (or relying on a comment that "the label carries it") means the asterisk never renders. The asterisk must be part of the `label` attribute text itself.

#### ❌ BAD: Asterisk in a separate hidden div

```html
<!-- The * is invisible — display:none hides it, and the label has no * -->
<lightning-input label="BRAND" value={brand} disabled></lightning-input>
<div class="required-asterisk">*</div>   <!-- display:none in CSS -->
```

#### ✅ GOOD: Inline asterisk in the label attribute

```html
<lightning-input label="BRAND *" value={brand} disabled></lightning-input>
```

> **Rule:** For required fields rendered as `lightning-input`, include the asterisk directly in the `label` string (e.g., `label="BRAND *"`). Do not rely on sibling DOM elements for visual indicators — they are fragile and easily hidden.
