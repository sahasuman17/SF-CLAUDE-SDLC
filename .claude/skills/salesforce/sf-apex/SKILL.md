...
| Standard | Rule |
|---|---|
| **Governor limits** | SOQL ≤ 100 per tx, DML ≤ 150, CPU ≤ 10,000 ms, heap ≤ 6 MB |
| **Bulkification** | operate on `List`/`Map` collections; never SOQL or DML inside loops |
| **No hardcoding** | use Custom Labels, Custom Metadata, or Custom Settings for configurable values |
| **Error handling** | use typed custom exception classes; never suppress with empty `catch` blocks |
| **Security** | apply `Security.stripInaccessible()` for FLS; use `WITH USER_MODE` in SOQL where appropriate |
| **Naming** | PascalCase classes, camelCase variables, ALL_CAPS constants |
| **Documentation** | Javadoc-style comment on every class and every public/global method signature |
