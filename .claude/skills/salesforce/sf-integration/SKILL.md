...
| Standard | Rule |
|---|---|
| **No hardcoding** | Named Credentials for endpoints; Custom Labels/Metadata for config values; never inline secrets or URLs |
| **Error handling** | handle all HTTP status codes explicitly; log request/response for observability |
| **Timeouts** | always set an explicit timeout on `HttpRequest`; default ≤ 10,000 ms |
| **Naming** | suffix callout service classes with `Service` or `Client`; mock classes with `Mock` |
