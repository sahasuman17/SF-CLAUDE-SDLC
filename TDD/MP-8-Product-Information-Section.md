# TDD: MP-8 — Support Cases: Product Information Section
...
The MP-8 page contains **only** these elements — no other sections exist:
...
### 6.1 Method Inventory (MP-8scope)

| Method                                         | Purpose                                                              | Story |
|------------------------------------------------|----------------------------------------------------------------------|-------|
| `findAssetBySerial(String serialNumber)`        | Searches Asset by serial number, returns asset data wrapper          | MP-8 |
| `getPartDescription(Id partId)`                 | Returns Part_Description__c for a Part__c record                     | MP-8 |
| `saveCaseAsDraft(Case caseRecord)`              | Upserts Case with Status = Draft, returns caseId + CaseNumber       | MP-8 |
...
Will be added in MP-5 when Case Information section is wired. Not called from MP-8 code.
...
See caseCreate.js for the full reactive property list.
...
When the user clicks **Save**, the LWC builds a Case sObject with **only Product Information fields** (section 4.4) and calls `saveCaseAsDraft`. No fields from other sections are included.
...
| File                                                          | Action | Purpose                                                  |
|---------------------------------------------------------------|--------|----------------------------------------------------------|
| `force-app/main/default/classes/CaseCreateController.cls`     | Create | Shared Apex controller: `findAssetBySerial`, `getPartDescription`, `saveCaseAsDraft` |
| `force-app/main/default/classes/CaseCreateController.cls-meta.xml` | Create | Apex metadata                                           |
| `force-app/main/default/classes/CaseCreateControllerTest.cls` | Create | 12 test methods, >= 80% coverage                        |
| `force-app/main/default/classes/CaseCreateControllerTest.cls-meta.xml` | Create | Test class metadata                                |
| `force-app/main/default/lwc/caseCreate/caseCreate.html`      | Create | Full UI including header, sections, footer              |
| `force-app/main/default/lwc/caseCreate/caseCreate.js`        | Create | Reactive JS controller with Apex imports                |
| `force-app/main/default/lwc/caseCreate/caseCreate.css`       | Create | Mockup-accurate styling                                 |
| `force-app/main/default/lwc/caseCreate/caseCreate.js-meta.xml` | Create | LWC metadata                                          |
